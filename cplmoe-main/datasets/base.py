    ...
    def __getitem__(self, index):
        ...
        for word, tag in nltk.pos_tag(nltk.tokenize.word_tokenize(sentence)):
            ...
            if word in self.keep_vocab:
                ...
                words.append(word)
        if len(words) == 0:
            # Fallback for all-OOV or empty-tokenized query
            pad_id = self.vocab['w2id'].get('<PAD>', 0)
            pad_vec = self.vocab['id2vec'][pad_id].astype(np.float32)
            words_id = [pad_id]
            weights = [1.0]
            words_feat = [pad_vec, pad_vec]  # start token + one placeholder token
        else:
            words_id = [self.keep_vocab[w] for w in words]
            words_feat = [
                self.vocab['id2vec'][self.vocab['w2id'][words[0]]].astype(np.float32)
            ]  # placeholder for the start token
